...
def load_detectron2_model(config_path, model_weights_path, device='cuda'):
    """
    Loads the trained Detectron2 model with adjusted confidence threshold.
    """
    cfg = get_cfg()
    cfg.merge_from_file(config_path)
    cfg.MODEL.WEIGHTS = model_weights_path
    cfg.MODEL.DEVICE = device
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.7  # Increased from 0.5 to 0.7
    cfg.MODEL.ROI_HEADS.NUM_CLASSES = 1  # Ensure this matches your training
...
def initialize_mediapipe():
    """
    Initializes Mediapipe Pose estimator.

    Returns:
        pose (mp.solutions.pose.Pose): Mediapipe Pose estimator.
    """
...
def visualize_results(image_bgr, instances, human_bboxes, all_keypoints):
    ...
    # Display the image
    cv2.imshow("Segmentation Results", image_bgr)
    cv2.waitKey(0)
    cv2.destroyAllWindows()


def run_inference(test_image_path, config_path, model_weights_path, yolo_model_path='yolov8n.pt'):
    # Initialize models
    ...
    if image_bgr is None:
        raise ValueError(f"Unable to read image at {test_image_path}")
    image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)

    # Detect humans with YOLOv8
    human_bboxes = detect_humans_yolov8(yolo_model, image_rgb)
    if len(human_bboxes) == 0:
        print("No humans detected. Exiting inference.")
        return

    # Predict keypoints with Mediapipe
    all_keypoints = get_keypoints_mediapipe(pose_estimator, image_rgb, human_bboxes)

    # Create masks
    bbox_mask = create_bbox_mask(image_rgb.shape, human_bboxes)
    kp_heatmap = create_keypoint_heatmap(image_rgb.shape, all_keypoints, radius=5)

    # Create multi-channel image
    combined_image = create_multi_channel_image(image_rgb, bbox_mask, kp_heatmap)
    ...
    instances = detectron2_inference(detectron_model, input_tensor, image_rgb.shape)
    print("instances")

    # Visualize the results
    visualize_results(image_bgr, instances, human_bboxes, all_keypoints)


test_image_path = "/home/karuppia/Documents/spotdata/data/images/left_frame_10316.jpg"  # Replace with your image path
...
run_inference(test_image_path, config_path, model_weights_path, yolo_model_path)
